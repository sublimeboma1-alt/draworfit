...
export function RegisterPage({ navigate }) {
  ...
  return <AuthFrame title="Creer un compte" subtitle="Vos achats seront rattaches a ce compte." error={error}><form onSubmit={submit}>
    <div className="field-row"><label>Prenom<input required value={form.first_name} onChange={change('first_name')} /></label><label>Nom<input required value={form.last_name} onChange={change('last_name')} /></label></div>
    ...
    <label>Numero de telephone<input required type="tel" value={form.phone_number} onChange={change('phone_number')} /></label>
    <label>Universite <small>(facultatif)</small><input value={form.university} onChange={change('university')} /></label>
    <label>Pays d'origine<input required value={form.country_of_origin} onChange={change('country_of_origin')} placeholder="Ex. Sénégal" /></label>
